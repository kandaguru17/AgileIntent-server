...
public class BacklogController {
	...
	public ResponseEntity<List<ProjectTask>> getAllProjectTasks(@PathVariable String projectIdentifier) {
		List<ProjectTask> allProjectTask = projectTaskService.getAllProjectTasks(projectIdentifier.toUpperCase());
	...
	public ResponseEntity<?> getProjectTaskById(@PathVariable String projectIdentifier,
		...
		ProjectTask foundProjectTask = projectTaskService.getProjectTaskByProjectTaskSequence(
				projectIdentifier.toUpperCase(), projectTaskSequence.toUpperCase());
		return new ResponseEntity<ProjectTask>(foundProjectTask, HttpStatus.OK);
	}
	
	
	@PatchMapping({"/{projectIdentifier}/projectTask/{projectTaskSequence}"})
	public ResponseEntity<?> updateProjectTask(@Valid @RequestBody ProjectTask updatedProjectTask,@PathVariable String projectIdentifier,
			@PathVariable String projectTaskSequence){
		
		
		
		ProjectTask returnedProjectTask=projectTaskService.updateProjectTask(projectIdentifier, projectTaskSequence, updatedProjectTask);
	...
	public ResponseEntity<?> deleteProjectTask(@PathVariable String projectIdentifier,
		...
		projectTaskService.deleteProjectTask(projectIdentifier, projectTaskSequence);
